...
| Phase 6 | **モジュール化** | 未着手 | ロジックを共有モジュールとして切り出し |
...
| 順番 | 機能 | 内容 | 方針 |
|------|------|------|------|
| 1 | **ボイシングポジション切替** | ボイシングボックスの番号（白い四角）をタップ→次のポジション候補に切り替え。同じコードでもルート位置によって押さえ方が変わるため、複数候補を順番に表示する | 番号ラベルを点滅させてタップ可能であることを示す。タップで`selectedVoicingBoxIdx`を循環 |
| 2 | **音源実装（エレピ）** | WebAudioFontでElectric Piano音源を導入 | JSファイル1つ追加、MIDIノート番号をそのまま使用。ベロシティレイヤーなし（音量制御のみ）。不満が出たらサンプラー方式に切替 |
| 3 | **シェル+テンション** | シェルボイシング状態でパッドをタップして9th/11th/13th等のテンションを追加 | 実際の演奏に近い操作感。シェル音 + タップしたテンション音が鳴る |
| 4 | **デザイン** | UIの見た目・操作性の改善 | 実機能が揃ってからデザインを詰める |
