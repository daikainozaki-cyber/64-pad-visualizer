...
**Spring Reverb 現在地（2026-04-11）**: worklet に最小の effect-chain abstraction を追加済み。`legacy_twin_send` と `pre_tremolo_main` を分離し、Twin 系は従来 send-return、DI/Suitcase 系は `main tone + mechanical noise -> spring -> tremolo` の実験経路を持てる。比較用に `Pad Sensei MK1 Spring EXP` preset を追加し、通常の `Pad Sensei MK1` は non-spring のまま保持した。release tail 切断バグは修正済み。単一 mono spring core では release 後の delayed whining が消えなかったため、`spring-reverb-processor.js` を下敷きにした **2-spring bank** (`Td=0.066 / 0.082`, `ah=0.60 / 0.58`) を `epiano-worklet-processor.js` へ移植した。さらに、DI loudness-match 後を spring send に使っていたこと、acoustic `mechanicalNoiseSum` を spring input に混ぜていたこと、`rhodesLevel` が spring device 前にあったことを修正し、routing の大きい誤りはほぼ潰れた。現状は `Spring EXP` を tone-spring 診断枝として使い、`linear` / `dispersion_only` / `dispersion_hf` / `dispersion_resonator` を比較した結果、`spring reverb` としてはまだ 0 点だが、`dispersion_resonator` で小さい空間感は残り、release の質の違うノイズはかなり減った。結論として、主問題は routing より **spring core / late-tail quality** にあり、次は定性的評価の再整理と、空間・merge・spring character を分けた次手法の検討に移る。参考: [[notes/permanent/2026/スプリングリバーブ開発では空間とspring characterとmergeを分離して設計する]] / [[notes/permanent/2026/AccutronicsやBelton系のBBDスプリングは音響心理と物理の橋として読むべきである]] / [[notes/permanent/2026/スプリングリバーブらしさを定性的に分解しBBD的成立条件へ落とすと軽量実装の道が開ける]] / [[notes/permanent/2026/スプリングリバーブの真物理モデリング商用実装はSoftube一社のみで学術論文ベースは世界的に珍しい]]
