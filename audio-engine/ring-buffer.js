...
// Command format: [track (float), sampleIndex (float), velocity (float), pitchRatio (float)]
// 4 floats per command, capacity = RING_CAPACITY commands.

var RING_FLOATS_PER_CMD = 4;
var RING_CAPACITY = 64; // max queued commands
var RING_DATA_FLOATS = RING_CAPACITY * RING_FLOATS_PER_CMD; // 256
// Layout: [writeHead (1 int), readHead (1 int)] + [data (256 floats)]
var RING_HEADER_BYTES = 8; // 2 x Int32
var RING_DATA_BYTES = RING_DATA_FLOATS * 4; // 256 x Float32
...
function ringWrite(ring, track, sampleIndex, velocity, pitchRatio) {
  ...
  ring.data[offset]     = track;
  ring.data[offset + 1] = sampleIndex;
  ring.data[offset + 2] = velocity;
  ring.data[offset + 3] = pitchRatio;
...
function ringDrain(ring, callback) {
  ...
  while (r !== w) {
    ...
    callback(
      ring.data[offset] | 0,       // track (int)
      ring.data[offset + 1] | 0,   // sampleIndex (int)
      ring.data[offset + 2],       // velocity (float)
      ring.data[offset + 3]        // pitchRatio (float)
    );
    r = (r + 1) % RING_CAPACITY;
  }
